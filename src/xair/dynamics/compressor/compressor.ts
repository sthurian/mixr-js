...
import { DynamicsFilter, DynamicsFilterDependencies } from '../filter/filter.js';
...
export const createCompressor = (dependencies: CompressorDependencies): Compressor => {
  const { oscClient, oscBasePath, createDynamicsFilter } = dependencies;
  const oscBaseAddress = `${oscBasePath}/dyn`;
  ...
  const filter = createDynamicsFilter({ oscBasePath, oscClient, dynamicsType: 'compressor' });
