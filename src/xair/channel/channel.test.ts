...
suite('Channel', () => {
    ...
    test('updates the name of a channel', async ()=>{
        ...
        assert.strictEqual(set.calledOnceWithExactly('/ch/01/config/name', [{ type: 'string', value: 'test-channel' }]), true);
    });
    ...
    test('fetches the color of a channel', async ()=>{
        ...
        const result = await channel.getConfig().fetchColor();
    ...
    test('updates the color of a channel', async ()=>{
        ...
        await channel.getConfig().updateColor('Green');
        assert.strictEqual(set.calledOnceWithExactly('/ch/01/config/color', [{ type: 'integer', value: 2 }]), true);
    });
